...
const StudentPortal = () => {
  ...
  const handleRequestSubmitted = (newRequest) => {
    ...
    toast.success(
      "Request submitted! Please download the form and get it signed.",
    );
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-5">
            <img
              src="/iitp-logo.png"
              alt="IITP"
              className="h-20 w-20 object-contain rounded-xl bg-white p-1.5"
            />
            <div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <div className="flex flex-col gap-1">
                          {request.items?.map((item, idx) => (
                            <h3
                              key={idx}
                              className="font-semibold text-[var(--theme-text)]"
                            >
                              {item.itemType}{" "}
                              <span className="text-[var(--theme-text-muted)] text-xs ml-1">
                                x{item.quantity}
                              </span>
                            </h3>
                          ))}
                      ...
                      <span
                        className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium ${getStatusColor(request.status)}`}
                      >
                    ...
                    <div className="flex justify-between items-center mt-2">
                      <div>
                        ...
                        {request.status === "Pending" && (
                          <div className="mt-2 flex items-center gap-2 bg-red-500/20 border border-red-500/30 rounded-lg px-3 py-2">
                            <p className="text-xs text-red-500 font-bold">
                              Get signatures & report to CC Office
                            </p>
                          </div>
                        )}
                      </div>
